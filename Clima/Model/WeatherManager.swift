...
struct WeatherManager {
    ...
    let weatherURL = "https://api.openweathermap.org/data/2.5/weather?appid=7c354e180fdf090487f9806cdd5b7ac2&units=metric"
        ...
        if let url = URL(string: urlString) {
            ...
            let task = session.dataTask(with: url) { (data, response, error) in
                ...
                if let saveData = data {
                    if let weather = self.parseJSON(weatherData: saveData) {
